...
const pickImageUrl = (raw: Record<string, unknown>): string => {
  ...
  if (imagesObject.data) {
    const firstData = toObject(imagesObject.data)
    const attributes = toObject(firstData.attributes)
    return normalizeImageUrl(
      toStringValue(attributes.url) || toStringValue(firstData.url),
    )
  }

  return normalizeImageUrl(toStringValue(imagesObject.url))
}
...
const createUrl = (path: string, params: URLSearchParams): string => {
...
export const getProducts = async (
  ...
  const query = createBaseParams()

  query.set('pagination[page]', String(page))
  query.set('pagination[pageSize]', String(pageSize))

  if (search) {
    query.set('filters[title][$containsi]', search)
  }

  if (categoryId) {
    query.set('filters[productCategory][id][$eq]', String(categoryId))
  }

  if (ids?.length) {
    ids.forEach((id, index) => {
      query.append(`filters[id][$in][${index}]`, String(id))
    })
  }

  const response = await fetch(createUrl('/products', query), {
