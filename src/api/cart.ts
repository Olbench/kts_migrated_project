import { API_BASE_URL, ENABLE_CART_SYNC } from '@/config/api'
...
export const getCartItems = async (): Promise<CartApiItem[]> => {
  ...
  const response = await fetch(`${API_BASE_URL}/cart`, { cache: 'no-store' })

  if (!response.ok) {
    throw new Error('Failed to load cart')
  }

  const rawItems = toRawItems(await response.json())
  const parsed = rawItems.map(parseCartEntry).filter((entry): entry is ParsedCartEntry => entry !== null)
...
export const addCartProduct = async (payload: CartMutationPayload): Promise<void> => {
  if (!ENABLE_CART_SYNC) {
    return
  }
...
export const removeCartProduct = async (payload: CartMutationPayload): Promise<void> => {
  if (!ENABLE_CART_SYNC) {
    return
  }
